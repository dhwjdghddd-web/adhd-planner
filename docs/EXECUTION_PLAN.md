...
### T11 — 보류 항목 연계 (i18n·도움말·구글로그인·디자인 토큰)  〔규모 L〕 [REVIEW] (부분 완료 — 도움말만)
- **목표**: 기존 보류 과제를 신규 작업과 충돌 없이 통합.
- **변경**: 영어 i18n 도입 + 휴식 문구 영어 격언 분기(`rest_quotes`를 로케일별 리스트로) — 미착수. **도움말 페이지 — ✅ 완료.** 구글 로그인 업그레이드(연결-not-로그인 설계) — 확인해보니 **이미 구현되어 있었음**(`auth_service.dart`의 `linkGoogle`, 설정 화면의 'Google 연결' 버튼). 디자인 토큰 리스타일 — 미착수.
- **파일**: 신규 `lib/features/help/help_page.dart`(아코디언 9섹션: 오늘 화면·구간 관리·지금(Focus)·메모·체크인·알람·알림·스트릭·설정·FAQ), `settings_page.dart`(맨 위 '도움말' 진입 행).
- **검증**: 위젯테스트(섹션 표시·펼침·진입). 전체 통과.
- **커밋**: `추가: 도움말 페이지 (T11 일부)`.
- **실행 결과 (2026-06-29, Sonnet 4.6 구현)**: 사용자가 4개 항목 중 "2부터"(도움말)를 지정. 기존 보류 메모(11일 전)는 '루틴' 엔티티 등 옛 설계 기준이라 outdated — Block merge로 Routine이 사라지고 T1~T10에서 추가된 기능(메모 재표면, 포커스 타이머, 다음 한 행동, MIT, 체크인, 수면 와인드다운, 알림 채널)까지 반영해 콘텐츠를 전부 새로 작성. 구글 로그인은 점검 중 이미 동작하는 걸 확인해 보류 메모를 제거(메모리 outdated 사례). i18n·디자인 토큰은 여전히 미착수 보류. test 305→308(+3).
