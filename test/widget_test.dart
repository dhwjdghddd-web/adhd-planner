...
void main() {
  ...
  testWidgets('App boots and shows the circular planner home', (tester) async {
    ...
    await repo.saveSettings(
      const AppSettings.defaults().copyWith(onboardingComplete: true),
    );
  ...
  testWidgets(
    'a pending alarm alert set before the app even starts opens once it is ready '
    '(cold start)',
    (tester) async {
      final repo = FakePlannerRepository();
      await repo.saveSettings(
      ...
      expect(
        find.descendant(
          of: find.byType(AlarmScreen),
          matching: find.text('약 먹기'),
        ),
        findsOneWidget,
      );
  ...
  testWidgets(
    'a pending alarm alert that arrives while the app is already open also opens it',
    (tester) async {
      final repo = FakePlannerRepository();
      await repo.saveSettings(
        const AppSettings.defaults().copyWith(onboardingComplete: true),
  ...
  testWidgets('_AchievementRecorder banks settled past days but never today', (
    tester,
  ) async {
    final repo = FakePlannerRepository();
    ...
    await repo.saveMicroStepProgress(
      MicroStepProgress.today('s1', const [0, 1]),
    );
    await repo.saveMicroStepProgress(
      MicroStepProgress.today('s1', const [0, 1], at: yesterday),
    );
    ...
    final banked = (await repo.watchAchievedDays().first)
        .map((d) => d.dateKey)
        .toSet();
  ...
  testWidgets(
    ...
    'not the full-screen 100% celebration',
    (tester) async {
      final repo = FakePlannerRepository();
      await repo.saveSettings(
        const AppSettings.defaults().copyWith(onboardingComplete: true),
      ...
      await repo.upsertSegment(
        _block(id: 's1', name: '약 먹기', microSteps: const ['a', 'b', 'c', 'd']),
      );
      await repo.saveMicroStepProgress(
        MicroStepProgress.today('s1', const [0, 1]),
      );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await repo.saveSettings(
        AppSettings.defaults().copyWith(
          onboardingComplete: true,
          lastPartialCelebratedDate: dayKeyFor(),
        ),
      );
      await repo.upsertSegment(
        _block(id: 's1', name: '약 먹기', microSteps: const ['a', 'b', 'c', 'd']),
  ...
  testWidgets(
    ...
    '(whole-block fallback achievement is not a "halfway")',
    (tester) async {
      final repo = FakePlannerRepository();
      await repo.saveSettings(
        const AppSettings.defaults().copyWith(onboardingComplete: true),
  ...
  testWidgets(
    ...
    '(un-checking back to 50% must not downgrade to a halfway nudge)',
    (tester) async {
      final repo = FakePlannerRepository();
  ...
  testWidgets(
    ...
    'line in the full-screen celebration',
    (tester) async {
      final repo = FakePlannerRepository();
      ...
      await repo.saveAchievedDay(
        AchievedDay.forDay(now.subtract(const Duration(days: 1))),
      );
      await repo.saveAchievedDay(
        AchievedDay.forDay(now.subtract(const Duration(days: 2))),
      );
      await repo.upsertSegment(
        _block(id: 's1', name: '약 먹기', microSteps: const ['a', 'b']),
      );
  ...
  testWidgets(
    ...
    'lastCelebratedDate into the *new*, unrelated account)',
    (tester) async {
      ...
      await oldRepo.upsertSegment(
        _block(id: 's1', name: '약 먹기', microSteps: const ['a', 'b']),
      );
      await oldRepo.saveMicroStepProgress(
        MicroStepProgress.today('s1', const [0, 1]),
      );
      ...
      final container = ProviderContainer(
        overrides: [
          plannerRepositoryProvider.overrideWith(
            (ref) => ref.watch(repoHolder),
          ),
        ],
      );
